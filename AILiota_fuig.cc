...
#define PLAYER_NAME                 Liiu_fuig
#define PLAYER_NAME_STRING          "Liiu_fuig"

#define PLAYER_SAFE_RADIUS          7
#define BONUS_SAFE_NUM_VERTEX       2
#define PLAYER_SAFE_AREA            20
...
struct PLAYER_NAME : public Player { 
     ...
     *
     */
    bool block_NWme(int &root, const int num)
    {
        int unbloked = 0;
    ...
    void transformer(const vector<int> &neighbours, vector<int> &empty_neighbours, Movement &movement, bool &mine)
    {
        ...
        //vector<int> non_bikes_bikes;
        vector<int> non_bikes_neigh;
        
        /*for(int i = 0; i < int(neighbours.size()); ++i)
        {
            int _id = neighbours[i];
            if(not_obstacle(_id))
            {
                int cont = 0;
                if(is_available(vertex(_id)))
                {
                    if(vertex(_id).bonus == Ghost) ghost = true;
                    vector<int> neighbours6 = vertex(_id).neighbours;
                    for(int j = 0; j < int(neighbours6.size()); ++j)
                    {
                        int id6 = neighbours6[j];
                        if(not_obstacle(id6))
                        {
                            if(is_available(vertex(id6))) 
                            {
                                vector<int> neighbours7 = vertex(id6).neighbours;
                                for(int z = 0; z < int(neighbours7.size()); ++z)
                                {
                                    int id7 = neighbours7[z];
                                    if(not_obstacle(id7) and is_available(vertex(id7))) ++cont;
                                }
                            }
                        }
                    }
                }
                if(cont > 0) non_bikes_neigh.push_back(_id);
                else bikes_neigh.push_back(_id);
            }
        }*/
        ...
        {
            if(block_NWme(non_bikes_neigh[0], PLAYER_SAFE_RADIUS))
            {
    ...
    void play ()
    {
        ...
        for(int i = 0; i < (int)my_bikes.size(); ++i)
        {
            ...
            if(has_turn())
            {
                if(NWme.ghost_duration < 0 and (block_(NWme.vertex, PLAYER_SAFE_AREA) or not_bikes())) ziga_zaga(movement);
                else 
                {
     ...
     *
     */
    bool block_(int &root, const int num)
    {
        int unbloked = 0;
        ...
        while(not pending_.empty())
        {
            ...
            {
                int id3 = neighbours3[i];
                if(vertex(id3).bike != -1) return false;
                if(not mark_[id3] and vertex(id3).wall == -1 and vertex(id3).bike == -1) 
                {
                    mark_[id3] = true;
    ...
    bool have_obstable(int id)
    {
        vector<int> neighbours8 = vertex(id).neighbours;
        for(int i = 0; i < int(neighbours8.size()); ++i)
        {
            int id8 = neighbours8[i];
            if(vertex(id8).wall != -1) return true; 
        }
        return false;
    }
     ...
     *
     */
    void ziga_zaga(Movement &movement)
    {
        bool _found = false;

        Marks _mark_edo_tensei;
        _mark_edo_tensei = Marks(nb_vertices(), false);  // welcome to hell, evil ! 
        _mark_edo_tensei[NWme.vertex] = true;

        Back_to_Dec _back_ghost;
        _back_ghost = Back_to_Dec(nb_vertices(), -2);
        _back_ghost[NWme.vertex] = -1;

        queue<Vertex> _edo_tensei;
        _edo_tensei.push(vertex(NWme.vertex));

        int last_ = -1;

        while(not _edo_tensei.empty() and not _found)
        {
            Vertex u = _edo_tensei.front();
            _edo_tensei.pop();
            ...
            if(have_obstable(u.id)) _found = true;

            for(int i = 0; i < int(neighbours5.size()); ++i)
            {
                int _id5 = neighbours5[i];
                if(not _mark_edo_tensei[_id5] and not_obstacle(_id5)) 
                {
                    _mark_edo_tensei[_id5] = true;
                    _edo_tensei.push(vertex(_id5));
                    _back_ghost[_id5] = u.id;
                }
            }
        }

        int _v_edo_tensei = NWme.vertex;
        int _pos = last_;

        while(_back_ghost[_pos] != -1)
        {
            _v_edo_tensei = _pos;
            _pos = _back_ghost[_pos];
        }

        movement.next_vertex = _v_edo_tensei;
