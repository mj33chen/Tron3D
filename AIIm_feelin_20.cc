...
#define PLAYER_NAME                 Im_feelin_20
#define PLAYER_NAME_STRING          "Im_feelin_20"
...
struct PLAYER_NAME : public Player { 
    ...
    void play ()
    {
        ...
        for(int i = 0; i < (int)my_bikes.size(); ++i)
        {
            ...
            if(has_turn())
            {
                ...
                if(NWme.bonus == Ghost) 
                {
                    if(NWme.ghost_duration > 0 or non_path(NWme.vertex)) 
                    {
    ...
    void resuscitate_NWme(Movement &movement)
    {
        ...
        while(not edo_tensei.empty() and not found)
        {
            ...
            else if(not mark_edo_tensei[u.id])
            {
                mark_edo_tensei[u.id] = true;
